...
const HomeFeaturedProducts = ({ title, topProducts }) => {
    ...
    const handleAddCart = (isAdded, id, name) => {
    ...
    const handleNotifyProduct = async (id, name) => {
        await verifyUser();
        let loggedUser = JSON.parse(localStorage.getItem("user"));
        if (loggedUser && loggedUser.token) {
            // dispatch(userVerifyAsync());
    ...
    const verifyUser = async () => {
        if (user && user.token) {
            ...
            await axios.get(`api/users/validateToken`, config)
                .then((response) => {
                    localStorage.setItem('user', JSON.stringify(response.data));
                })
                .catch((error) => {
                    if (error.response.data.message === 'Bad token') {
                        toast.error('You token got expired. Plesae login gain to continue with your account.');
                    }
    ...
    return (
        ...
        <section className="marketing bg-light">
            ...
            <div className="mx-1 row row-cols-2 row-cols-sm-2 row-cols-md-4 row-cols-lg-5 g-3 g-sm-3 g-md-4 g-lg-4">
                {products.map((product) => (
                    <div key={product._id} className="col p-1 m-0">
                        <div className="card product_card p-0 p-md-0 m-0 rounded-0 border-0" style={{ boxShadow: 'none' }}>
                            ...
                            {product.countInStock < 1 && <span className="position-absolute top-0 start-0 mt-3 ms-5 translate-middle badge bg-light-subtle text-muted rounded-pill text-uppercase z-index-1" style={{ zIndex: 1000 }}><small>Out of Stock</small></span>}
                            {product.newCollection && <span className="position-absolute top-0 start-0 mt-3 ms-4 translate-middle badge bg-warning-subtle text-dark rounded-pill text-uppercase z-index-1" style={{ zIndex: 999 }}><small>New</small></span>}
                            {/* </div> */}
