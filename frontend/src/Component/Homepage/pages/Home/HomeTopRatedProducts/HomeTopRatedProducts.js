...
const HomeTopRatedProducts = ({ title, topProducts }) => {
    ...
    const handleAddWishlist = async (id, name, category, price, image) => {
        await verifyUser();
        let loggedUser = JSON.parse(localStorage.getItem("user"));
        if (loggedUser && loggedUser.token) {
            dispatch(addWishlistItem({ wId: id, name: name, category: category, price: price, image: image }));
        } else {
            // toast.success(`Please login to save this notification.`);
            navigate('/login');
        }
    }
    ...
    const handleAddCart = async (isAdded, id, name) => {
        await verifyUser();
        let loggedUser = JSON.parse(localStorage.getItem("user"));
        if (loggedUser && loggedUser.token) {
            if (isAdded) {
                toast.warning(`${name} already in cart`);
            } else {
    ...
    const verifyUser = () => {
        if (user && user.token) {
            ...
            axios.get(`api/users/validateToken`, config)
                .then((response) => {
                ...
                {products.map((product) => (
                    <div key={product._id} className="col">
                        <div className="card bg-light product_card p-0 p-md-0 m-0 rounded-0" style={{ boxShadow: 'none' }}>
                            ...
                            {product.countInStock < 1 && <span className="position-absolute top-0 start-0 mt-3 ms-5 translate-middle badge bg-danger text-white rounded-pill text-uppercase fw-semibold z-index-1" style={{ zIndex: 1000 }}><small>Out of Stock</small></span>}
                            ...
                            {/* <img src={product.image} className="card-img-top bg-light" alt="card 1" /> */}
