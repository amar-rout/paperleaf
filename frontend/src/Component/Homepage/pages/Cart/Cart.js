...
import { getStatus, getError, getListCartItems, getCartCount } from "../../../../app/cartSlice";
import { useDispatch, useSelector } from "react-redux";

const Cart = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const getCartStatus = useSelector(getStatus);
    const getCartCounts = useSelector(getCartCount)
    const getCartError = useSelector(getError);
    const getCartItems = useSelector(getListCartItems);
    ...
    useEffect(() => {
        ...
        setCartCount(getCartCounts);
    }, [getCartItems, getCartCounts]);

    useEffect(() => {
        if (getCartStatus === "LOADING") {
            setLoading(true);
        }
        if (getCartStatus === "LOADED") {
            setCartItems(getCartItems);
            setLoading(false);
        }
        if (getCartStatus === "ERROR") {
            setLoading(false);
            setErrorMessage(getCartError);
        }
    }, [getCartStatus, getCartItems, getCartError, dispatch]);
    ...
    return (
        <HelmetProvider>
            ...
            <div className="container my-5">
                ...
                { getCartCounts === 0 ?
                    <>
                        <span className="fs-4 fw-semibold">Empty cart</span>
                    </>
                    :
                    <div className="row">
                        <div className="col-12 col-md-8">
                            <div className="pt-1 px-2 px-md-5">
                                <h5 className="pb-2 pt-md-2 my-4 mt-lg-5">My Cart<span className="fs-base fw-normal text-muted">({getCartCounts} items)</span></h5>
